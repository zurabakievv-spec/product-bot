...
# ---- helpers for Base64-encoded JSON storage ----
def ensure_dirs():
...
def safe_load_json(filename: str, default):
    """
    Load Base64-encoded JSON from data/<filename>.
    If file missing/empty/corrupt — return default and log warning.
    """
...
def safe_save_json(filename: str, data):
    """
    Save JSON as Base64-encoded string into data/<filename>.
    Overwrites atomically by writing to tmp file then renaming.
    """
    path = data_path(filename)
    try:
...
def init_storage():
    ...
    defaults = {
        ...
        PRODUCTS_FILE: [],
        ORDERS_FILE: [],
        ADMINS_FILE: [707877919],  # keep example admin
    }
...
def add_order(client_name, phone, comment, cart_items, total):
    orders = load_orders()
    order = {
        "id": len(orders) + 1,  # stable incremental id; acceptable but can switch to uuid4()
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # admin quick actions (start corresponding conversation by returning state)
    if text == "➕ Добавить товар" and is_admin(update.effective_user.id):
        # start add-product conversation by asking name
        await update.message.reply_text("Введите название товара:")
        return ADD_PRODUCT_DESC  # next state will process description (we reuse entry->state pattern)
    if text == "➕ Добавить категорию" and is_admin(update.effective_user.id):
        await update.message.reply_text("Введите название новой категории:")
        return ADD_CATEGORY_NAME
    if text == "👤 Добавить админа" and is_admin(update.effective_user.id):
        await update.message.reply_text("Введите Telegram ID нового администратора:")
        return ADD_ADMIN_ID
    if text == "📋 Список товаров" and is_admin(update.effective_user.id):
        return await list_products_admin(update, context)
    if text == "❌ Удалить товар" and is_admin(update.effective_user.id):
...
async def show_product_card(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not products or index < 0 or index >= len(products):
        await (update.callback_query.message if update.callback_query else update.message).reply_text("Ошибка: товар не найден.", reply_markup=main_keyboard())
        return ConversationHandler.END
    ...
    if photo_path and os.path.exists(photo_path):
        with open(photo_path, "rb") as ph:
            if update.callback_query:
                await update.callback_query.message.reply_photo(photo=ph, caption=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.HTML)
            else:
                await update.message.reply_photo(photo=ph, caption=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.HTML)
    else:
        if update.callback_query:
...
# Navigation & adding to cart (callback handler)
async def nav_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "nav_next":
        context.user_data["current_index"] = min(len(context.user_data.get("cat_products", [])) - 1, context.user_data.get("current_index", 0) + 1)
        return await show_product_card(update, context)
    ...
    if action.startswith("add|"):
        ...
        await query.message.reply_text(f"Сколько штук добавить? (доступно: {product.get('stock',0)})", reply_markup=ReplyKeyboardMarkup([["Отмена"]], resize_keyboard=True))
        return ADD_TO_CART_QTY

    # nav_none or unknown: do nothing
    return ConversationHandler.END

# Add to cart (message handler)
async def add_to_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if existing:
        ...
        cart.append({"id": pid, "name": product.get("name"), "price": product.get("price"), "quantity": qty})
    context.user_data["cart"] = cart
    await update.message.reply_text(f"✅ {product.get('name')} × {qty} добавлено в корзину!", reply_markup=main_keyboard())
    return ConversationHandler.END
...
async def view_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton("✅ Оформить заказ", callback_data="checkout")], [InlineKeyboardButton("🗑 Очистить корзину", callback_data="clear_cart")]]
...
async def ask_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = ReplyKeyboardMarkup([[KeyboardButton("📱 Поделиться номером", request_contact=True)], ["Отмена"]], resize_keyboard=True, one_time_keyboard=True)
...
async def ask_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Комментарий к заказу (необязательно):", reply_markup=ReplyKeyboardMarkup([["Пропустить"], ["Отмена"]], resize_keyboard=True))
    return ASK_COMMENT
...
async def ask_comment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    order = add_order(context.user_data.get("client_name"), context.user_data.get("phone"), comment, cart, total)
    # send order to group
    await context.bot.send_message(GROUP_CHAT_ID, fmt_order_message(order), parse_mode=ParseMode.HTML)
    await update.message.reply_text("✅ Заказ оформлен! Мы свяжемся с вами в ближайшее время.", reply_markup=main_keyboard())
    context.user_data["cart"] = []
    return ConversationHandler.END

# Admin: list/add/delete products, add category, add admin
async def list_products_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for p in products:
        lines.append(f"ID: {p.get('id')} | {escape(str(p.get('name')))} | {p.get('price')}₽ | Остаток: {p.get('stock')}")
...
def main():
    ...
    # ConversationHandlers: we keep entry_points minimal and states clear
    add_product_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(r"^➕ Добавить товар$") & filters.User(func=lambda u: True), add_product_name)],
        states={
            ...
            ADD_PRODUCT_CATEGORY: [CallbackQueryHandler(add_product_category, pattern=r"^newcat\|")],
            ADD_PRODUCT_PHOTO: [
    ...
    order_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(cart_action, pattern=r"^(checkout|clear_cart)$")],
        states={
    ...
    cart_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(nav_product, pattern=r"^add\|")],
        states={
    ...
    add_category_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(r"^➕ Добавить категорию$"), add_category_name)],
    ...
    add_admin_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(r"^👤 Добавить админа$"), add_admin_id)],
        states={ADD_ADMIN_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_admin_id)]},
        fallbacks=[CommandHandler("cancel", cancel)],
        allow_reentry=True,
    )

    delete_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(r"^❌ Удалить товар$"), delete_product)],
        states={DELETE_PRODUCT_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, delete_product)]},
        fallbacks=[CommandHandler("cancel", cancel)],
        allow_reentry=True,
    )
    ...
    app.add_handler(CallbackQueryHandler(show_category_products, pattern=r"^cat\|"))
    # handle navigation callbacks (nav_prev, nav_next, back_to_cats, add|)
    app.add_handler(CallbackQueryHandler(nav_product, pattern=r"^(nav_prev|nav_next|nav_none|back_to_cats|add\|)"))
    app.add_handler(CallbackQueryHandler(cart_action, pattern=r"^(checkout|clear_cart)$"))
